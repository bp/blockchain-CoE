...
const vkJson = JSON.parse(
  fs.readFileSync(`./proving-files-dev/decrypt-tally-hash_vk.key`, 'utf-8')
);
...
const transferToken = async (contractAddress, toAddress, abi) => {
  try {
    const provider = new ethers.providers.JsonRpcProvider(JSON_RPC);
    var signer = new ethers.Wallet(ADMIN_PRIVATE_KEY, provider);
...
// this script is used to deploy contract to Ethereum ropsten testnet
main();
