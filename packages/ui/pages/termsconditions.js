...
export default function TermsConditions({ termsConditions }) {
  React.useEffect(() => {
  ...
  return (
    <section
      className={[styles.termsConditionsSection, getSectionStyle(alerts, cookieConsent)].join(' ')}>
      <div className={['container', styles.faqSection].join(' ')}>
        <h2 className={styles.heading}>Terms & Conditions</h2>
        <div className={styles.termsConditions}>
          {termsConditions &&
            termsConditions.map(({ title, content }, index) => (
              <div key={index}>
...
TermsConditions.propTypes = {
  termsConditions: PropTypes.array
};
